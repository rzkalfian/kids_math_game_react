    ...
    <main className="home-screen">
      ...
      <div className="home-content">
        ...
        <section className="welcome">
          ...
          <h1>Petualangan angka<br /><span>dimulai di sini.</span></h1>
          <p className="welcome__copy">Pilih permainan favoritmu. Setiap jawaban benar membuat kemampuan matematikamu semakin kuat.</p>
        </section>
          ...
          <div className="game-grid">
            {GAMES.map(({ id, title, description, icon: Icon, color }, index) => (
              <button className={`game-option game-option--${color}`} type="button" key={id} onClick={() => selectMode(id)}>
                ...
                <span className="game-option__arrow"><ArrowRight aria-hidden="true" /></span>
              </button>
            ))}
          </div>
        </section>
        ...
        <footer className="home-footer">
          ...
          <div className="progress-dots" aria-hidden="true"><i /><i /><i /></div>
        </footer>
      </div>
    </main>
  );
