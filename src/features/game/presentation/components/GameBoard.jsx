...
import { ArrowLeft, Check, ChevronDown, Minus, Plus, RefreshCw, Star, Trash2, Volume2, VolumeX, X } from 'lucide-react';
...
export function GameBoard({ mode, randomNumberRepository, onBack, audio }) {
  ...
  function handleRoll(slot) {
    ...
    roll(slot, mode === 'dice' ? audio.diceShakeDurationMs : 0, supportsLevels ? level : undefined);
  }
        ...
        <div className="game-settings">
          ...
          {supportsLevels && (
            <label className="level-select" title="Pilih level angka">
              <select value={level.id} onChange={(event) => selectLevel(LEVELS.find((option) => option.id === Number(event.target.value)))} disabled={isAnswered}>
                {LEVELS.map((option) => <option value={option.id} key={option.id}>{option.label} ({option.minimum}-{option.maximum})</option>)}
              </select>
